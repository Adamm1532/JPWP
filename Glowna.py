...
class Glowna(ctk.CTk):
    def __init__(self):
        ...
        for x in range(0,len(glb.historia1),2):
            label = ctk.CTkLabel(self.historia_scrol, text= glb.historia1[x], fg_color=glb.color_background)
            label.grid(row=x, column=1, padx=(10, 5), pady=10, sticky=ctk.W,)


            label2 = ctk.CTkLabel(self.historia_scrol, text= glb.historia1[x+1],fg_color=glb.color_background)
    ...
    def expand(self,x):
        ...
        label_czas = ctk.CTkLabel(self.historia_scrol, text= datetime.datetime.now(), fg_color=glb.color_background)
        label_czas.grid(row=x+1, column=1, padx=(10, 5), pady=10, sticky=ctk.W,)
        for button in self.buttons:
            button.config(image=self.arrow1_img)
        for label in self.labels:
            label.grid_remove()

    # Change the image in the clicked button
        self.buttons[x//2].configure(image=self.arrow2_img)
        self.labels[x].grid(row=x + 1, column=1, padx=(10, 5), pady=10, sticky=ctk.W)
