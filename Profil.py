...
class Profil(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        for idx, button_text in enumerate(change_texts, start=2):
            ...
            button = ctk.CTkButton(self.background_frame, image=self.Pen_img,
                                   ...
                                   text="")
            button.grid(row=idx, column=1, sticky="nsew", padx=50, pady=10)
        ...
        self.Profile_Button = ctk.CTkButton(self.background_frame, image=self.profile_img, text="",
                                            ...
                                            hover_color=glb.color_background,
                                            corner_radius=100)
        self.Profile_Button.place(relx=0.4, rely=0.25, anchor="center")
    ...
    def change_mode_dark(self):
        ...
        for label in self.change_labels:
            label.configure(fg_color=glb.color_background, text_color=glb.color_text)
        for button in self.change_buttons:
            button.configure(fg_color=glb.color_background, hover_color=glb.color_background)
        self.Back_img = ctk.CTkImage(dark_image=glb.back_button, size=(30, 30))
